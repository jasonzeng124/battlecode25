...
public class Soldier extends Unit {
    ...
    public void wander() throws GameActionException{
            ...
            if(rc.canSenseRobotAtLocation(ml)){
                ...
                if(rc.getNumberTowers() < 25){
                    ...
                    if(!badRuins.contains(ml)){
                        ...
                        checkGoodFocus: {
                            for(RobotInfo ri : rc.senseNearbyRobots(ml, 1, myTeam)){
        ...
        switch(focusType){
            case RUIN:
                if(distToFocus <= 1){
                    ...
                    for(RobotInfo ri : rc.senseNearbyRobots(focus, 1, myTeam)){
                        if(ri.type == UnitType.SOLDIER){
                            checkCanBuild = false;
                            break;
                        }
        ...
        //if you are on enemy territory, you might as well use the rest of your paint to explore and hit towers rather than to come back
        if(rc.getPaint() < 40){
            clearFocus();
    ...
    public void fight() throws GameActionException{
        ...
        for (int i = 8; --i >= 0; ) {
            ...
            if (rc.canMove(adjDirections[i])) {
                ...
                if (rc.getLocation().distanceSquaredTo(focus) > 9 && (rc.getActionCooldownTurns() < 20)){
    ...
    public void build() throws GameActionException{
        ...
        if(rc.getNumberTowers() == 25 && focus != null){
            clearFocus();
            curState = SoldierState.WANDER;
        }
        ...
        if(buildType == null){
            double chanceOfMoney = 1.0;
            if(rc.getNumberTowers() > 2){
                if(estimatedIncome <= 0){
                    chanceOfMoney = 1.0;
                }else{
                    chanceOfMoney = (30.0/(estimatedIncome));
                }
            }
